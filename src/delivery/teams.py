...
class TeamsGraphSender:
    ...
    def _card_to_html(self, payload: dict) -> str:
        ...
        return f'<attachment id="1"></attachment>'
...
class TeamsPowerAutomateSender:
    ...
    Authentication
    ──────────────
    The Power Platform HTTP trigger URL requires an Azure AD bearer token.
    This class obtains one automatically using MSAL device-code flow the
    first time, then caches the refresh token at ~/.jira_alerting_token.json
    so subsequent runs are silent.

    First run:  you'll see "Go to https://microsoft.com/devicelogin and enter
                code XXXXXXXX" — takes ~10 seconds.
    After that: runs silently, auto-refreshing the token as needed.
    """

    # Well-known public client — no app registration needed.
    # This is the Azure PowerShell client ID, usable by any tenant member.
    ...
    # ------------------------------------------------------------------
    # Token management (MSAL device-code flow)
    # ------------------------------------------------------------------
    ...
    def send(self, payload: dict[str, Any], recipient_email: str) -> bool:
        ...
        if not token:
            logger.error("Cannot send — failed to obtain Teams auth token")
            return False
